...
import com.sma.validator.TransaksiBiayaValidator;

/**
 * Controller untuk Transaksi Uang Masuk / Biaya Lain-lain
 * Menu ini adalah copy paste kembaran dari menu TransaksiPaymentController
 * Bedanya hanya saat input di mst_payment, tidak perlu input no_stt
 * 
 * @author Yusuf
 ...
 */
@Controller
@RequestMapping("/transaksi/biaya")
public class TransaksiBiayaController extends ParentController {

	@Autowired
	protected TransaksiBiayaValidator validator;
	...
	public String show(Model model, HttpServletRequest request) throws ParseException {
		...
		double total 	= dbService.selectListPaymentCount(search, dateFrom, dateTo); //total data
		...
		//model data
		model.addAttribute("listPayment", dbService.selectListPayment(search, dateFrom, dateTo, offset, rowcount));
		
		return "input_biaya_list";
	}
	
	//insert payment
	@RequestMapping(value="/insert", method=RequestMethod.GET)
	public String insert(@ModelAttribute("payment") Payment payment) {
		logger.debug("Halaman: Input Biaya, method: INSERT");
	...
	public String update(@ModelAttribute("payment") Payment payment, @PathVariable Integer id) {
		logger.debug("Halaman: Input Biaya, method: UPDATE");
	...
	public String save(@Valid @ModelAttribute("payment") Payment payment, BindingResult result, HttpServletRequest request, RedirectAttributes ra) throws MailException, MessagingException {
		logger.debug("Halaman:  Input Biaya, method: SAVE");
		...
		return "redirect:/transaksi/biaya"; 
