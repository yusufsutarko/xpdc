	...
	<div id="tabs-1">
		...
		<table class="gridTables">
			<caption>
				...
				<input type="text" name="search" value="${search}" title="Cari berdasarkan NOMOR PAYMENT / NOMOR STT / KETERANGAN / CARA BAYAR">
				...
				<td>
					<c:choose>
						<c:when test="${d.cancel eq 0}">
							<button onclick="window.location='${path}/transaksi/payment/update/${d.payment_id}'; return false;">Rubah</button>
						</c:when>
						<c:when test="${d.cancel eq 1}">
							<button onclick="window.location='${path}/transaksi/payment/update/${d.payment_id}'; return false;">Lihat</button>
						</c:when>
					</c:choose>
				</td>
			</tr>
		...
		<p>
			<button onclick="window.location='${path}/transaksi/payment/insert'; return false;">Tambah Baru</button>
		</p>
		</form>
	</div>
</div>
