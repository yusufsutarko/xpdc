...
<div class="tabs">
	<ul>
		<li><a href="#tabs-1">Home &raquo; Master Supir</a></li>
	</ul>
	<div id="tabs-1">
		<form method="post">
		<table class="gridTables">
			<caption>
				Cari:
				<input type="text" name="search" value="${search}" title="Cari berdasarkan NAMA">
				Tampilkan:
			...
			<c:forEach items="${listSupir}" var="u" varStatus="s">
			<tr>
				<td>${u.id}</td>
				<td>${u.nama}</td>
				<td><fmt:formatDate value="${u.tgl_mulai}" pattern="dd-MM-yyyy"/></td>
				<td><fmt:formatDate value="${u.tgl_berakhir}" pattern="dd-MM-yyyy"/></td>
				...
				<td>
					<button onclick="window.location='${path}/master/supir/update/${u.id}'; return false;">Rubah</button>
				</td>
			</tr>
			</c:forEach>
		</table>
		<p>
			<button onclick="window.location='${path}/master/supir/insert'; return false;">Tambah Baru</button>
		</p>
		</form>
	</div>
</div>
