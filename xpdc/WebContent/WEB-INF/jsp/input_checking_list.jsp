				...
				<td>
					<c:choose>
						<c:when test="${d.cancel eq 0}">
							<button onclick="window.location='${path}/transaksi/checking/update/${d.id}'; return false;">Rubah</button>
<!-- 							<button onclick="cetak(${d.id}); return false;">Cetak</button> -->
						</c:when>
						<c:when test="${d.cancel eq 1}">
							<button onclick="window.location='${path}/transaksi/checking/update/${d.id}'; return false;">Lihat</button>
						</c:when>
					</c:choose>
				</td>
			</tr>
