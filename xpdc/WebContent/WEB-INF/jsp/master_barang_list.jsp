...
<div class="tabs">
	<ul>
		<li><a href="#tabs-1">Home &raquo; Master Barang</a></li>
	</ul>
	<div id="tabs-1">
		<form method="post">
		<table class="gridTables">
			<caption>
				Cari:
				<input type="text" name="search" value="${search}" title="Cari berdasarkan NAMA BARANG">
				Tampilkan:
			...
			<c:forEach items="${listBarang}" var="u" varStatus="s">
			<tr>
				<td>${u.id}</td>
				<td>${u.nama}</td>
				...
				<td>
					<button onclick="window.location='${path}/master/barang/update/${u.id}'; return false;">Rubah</button>
				</td>
			</tr>
			</c:forEach>
		</table>
		<p>
			<button onclick="window.location='${path}/master/barang/insert'; return false;">Tambah Baru</button>
		</p>
		</form>
	</div>
</div>
